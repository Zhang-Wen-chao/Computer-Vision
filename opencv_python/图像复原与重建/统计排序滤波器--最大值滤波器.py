        ...
        for j in range((n - 1) // 2, img_input.shape[1] - (n - 1) // 2):
            ...
            for x in range(-(m - 1) // 2, (m + 1) // 2):
                for y in range(-(n - 1) // 2, (n + 1) // 2):
                    ...
                    h += 1
            sortarray.sort()
            img_output[i - (m - 1) // 2, j - (n - 1) // 2] = sortarray[num - 1]
    return img_output

def add_salt_and_pepper_noise(image, prob):
    output = np.copy(image)
    num_salt = np.ceil(prob * image.size / 2)
    coords = [np.random.randint(0, i - 1, int(num_salt))
              for i in image.shape]
    output[coords[0], coords[1]] = 255

    num_pepper = np.ceil(prob * image.size / 2)
...
if __name__ == "__main__":
    # Generate a grayscale image of size 512x512 with salt and pepper noise
    image = np.ones((512, 512), dtype=np.uint8) * 127
    noisy_image = add_salt_and_pepper_noise(image, 0.02)  # 2% noise
    cv2.imshow("Noisy Image", noisy_image)

    # Apply custom max filter to the noisy image
    filtered_image_custom = max_filter(noisy_image, 7, 7)
