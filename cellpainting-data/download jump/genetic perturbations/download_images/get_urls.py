...
def download_urls(numbers):
    ...
    with open(file_name, 'a') as f:
        for number in numbers:
            url = f'https://phenaid.ardigen.com/api-jumpcpexplorer/gene-info/{number}'

            response = requests.get(url, headers=headers)
            data = response.json()

            folder = str(number)
            for image_url in data['gene_info']['images']:
                parts = image_url.split('/')
                filename = f"{parts[-3]}-{parts[-2]}-{parts[-1]}"
                f.write(f'{folder}::Images for {folder}::{filename}::{image_url}\n')

            for control_image_url in data['gene_info']['control_images']:
